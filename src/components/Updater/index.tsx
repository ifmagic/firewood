...
function showUpdateNotification(info: UpdateInfo, pendingUpdate: UpdateRef) {
  ...
  notification.info({
    ...
    btn: (
      <Space>
        ...
        <Button type="primary" size="small" onClick={() => startUpdate(key, pendingUpdate)}>
...
async function checkForUpdate(pendingUpdate: UpdateRef, manual = false) {
  try {
...
export default function Updater() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => checkForUpdate(pendingUpdate), 3000);
    const interval = setInterval(() => checkForUpdate(pendingUpdate), 5 * 60 * 60 * 1000);

    const unlistenPromise = listen('app://check-for-updates', async () => {
      await checkForUpdate(pendingUpdate, true);
    });
